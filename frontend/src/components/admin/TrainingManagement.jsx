...
export default function TrainingManagement() {
  ...
  const fetchTrainingPrograms = async () => {
    try {
      const response = await fetch('http://localhost:5000/training-management/training-programs');
      if (response.ok) {
  ...
  const handleProgramClick = (program) => {
    navigate('/dashboard_admin/training-progress-update', { state: { selectedProgram: program } });
  };
    ...
    <div style={{ fontFamily: FONT, minHeight: '100vh', padding: '0 2rem 2rem 2rem' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
          ...
          <h1 style={{ color: '#6D2323', fontWeight: 700, fontSize: 32, margin: 0 }}>Training Management</h1>
        </div>
        <p
          style={{
            color: '#666',
        ...
        >
          View and manage training programs' participant progress.
        </p>
      </div>
