...
export default function NotificationEmployee() {
  ...
  const getNotificationIcon = (message) => {
    ...
    if (msg.includes('success') || msg.includes('completed') || msg.includes('approved')) return <CheckCircle size={20} color={SUCCESS} />;
    if (msg.includes('error') || msg.includes('failed') || msg.includes('rejected')) return <AlertCircle size={20} color={ERROR} />;
  ...
  const getNotificationType = (message) => {
    ...
    if (msg.includes('success') || msg.includes('completed') || msg.includes('approved')) return 'success';
    if (msg.includes('error') || msg.includes('failed') || msg.includes('rejected')) return 'error';
  ...
  const markSelectedAsRead = async () => {
    ...
    try {
      ...
      const promises = selectedIds.map(id => 
        fetch(`http://localhost:5000/api/notifications/employee/${id}/read`, {
          method: 'POST',
          headers: {
  ...
  const markSelectedAsUnread = async () => {
    ...
    try {
      ...
      const promises = selectedIds.map(id => 
        fetch(`http://localhost:5000/api/notifications/employee/${id}/unread`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      const promises = selectedIds.map(id => 
        fetch(`http://localhost:5000/api/notifications/employee/${id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const markAsRead = async (notificationId) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/notifications/employee/${notificationId}/read`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const markAllAsRead = async () => {
    ...
    try {
      ...
      const promises = unreadNotifications.map(n => 
        fetch(`http://localhost:5000/api/notifications/employee/${n.id}/read`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const markAllAsUnread = async () => {
    ...
    try {
      ...
      const promises = readNotifications.map(n => 
        fetch(`http://localhost:5000/api/notifications/employee/${n.id}/unread`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  useEffect(() => {
    ...
    const user = JSON.parse(localStorage.getItem('user') || 'null');
    if (!user?.id) {
      console.warn('No user ID found, cannot establish SSE connection');
      return;
    }

    console.log(`🔌 Establishing SSE connection for employee user ${user.id}`);
    ...
    eventSource.onopen = () => {
      console.log('✅ SSE connection opened for employee notifications');
    };
  ...
  const deleteAll = async () => {
    try {
      ...
      const res = await fetch('http://localhost:5000/api/notifications/employee/delete-all', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <>
      ...
      <div style={{ 
        ...
        {/* Filter tabs with action buttons */}
        <div style={{
          display: 'flex',
